...
import { resolveProvidersForDisplay, formatRegistryAuthLabel } from '../registry/provider-catalog.js';
...
export function parseProvidersArgs(args: string[]): ParsedProvidersArgs {
  ...
  if (first === 'auth') {
    ...
    const authMethod: ProviderAuthMethod | undefined = native ? 'native' : broker ? 'broker' : undefined;
