...
const DSML_BLOCK_RE = new RegExp(
  `<${DSML_NOISE}DSML${DSML_NOISE}tool_calls>([\\s\\S]*?)<\\/${DSML_NOISE}DSML${DSML_NOISE}tool_calls>`,
  'i'
)
const DSML_INVOKE_RE = new RegExp(
  `<${DSML_NOISE}DSML${DSML_NOISE}invoke\\s+name="([^"]+)"[^>]*>([\\s\\S]*?)<\\/${DSML_NOISE}DSML${DSML_NOISE}invoke>`,
  'gi'
)
const DSML_PARAM_RE = new RegExp(
  `<${DSML_NOISE}DSML${DSML_NOISE}parameter\\s+name="([^"]+)"(?:\\s+string="(true|false)")?[^>]*>([\\s\\S]*?)<\\/${DSML_NOISE}DSML${DSML_NOISE}parameter>`,
  'gi'
)
...
export function splitToolUseId(id: string): { rawId: string; thoughtSignature?: string } {
  ...
  if (sep !== -1) {
    return {
      ...
      thoughtSignature: Buffer.from(id.slice(sep + TOOL_USE_SIG_SEP.length), 'base64url').toString(
        'utf8'
      ),
    }
  }
