...
export const VERTEX_ONE_M_MODEL_IDS = new Set(['claude-sonnet-4-6', 'claude-opus-4-6'])
...
export function loadVertexModelEntries(env: EnvLike = process.env): VertexModelEntry[] {
  ...
  try {
    ...
    const models = parsed
      .filter(
        (entry): entry is VertexModelEntry =>
          !!entry &&
          typeof entry === 'object' &&
...
function registerVertexCatalogAlias(
  byId: Map<string, ServerModelInfo>,
  alias: string,
