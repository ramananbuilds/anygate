...
import { splitToolUseId } from '../proxy/proxy-shared.js'
  ...
  for (const block of content as JsonRecord[]) {
    ...
    if (type === 'text' && typeof block.text === 'string') {
      ...
      if (typeof rawContent === 'string') {
        ...
        result = (rawContent as JsonRecord[])
...
function translateTools(tools: unknown): JsonRecord[] | undefined {
  ...
  for (const t of tools as JsonRecord[]) {
    ...
    decls.push({
      ...
      parameters: stripDraftMeta(
        normalizeJsonSchema(t.input_schema ?? { type: 'object', properties: {} })
      ),
    })
  }
