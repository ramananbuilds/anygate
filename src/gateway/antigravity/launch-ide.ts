...
export function isAntigravityIdeRunning(
  profileDir: string,
  ...
  return output
    .split('\n')
    .some(
      line => line.includes('Antigravity IDE.app') && line.includes(`--user-data-dir=${profileDir}`)
    )
}

export function isAntigravityAppRunning(
  profileDir: string,
  processList = defaultProcessList
): boolean {
  if (process.platform === 'win32')
  ...
  return output
    .split('\n')
    .some(
      line => line.includes('Antigravity.app') && line.includes(`--user-data-dir=${profileDir}`)
    )
}
