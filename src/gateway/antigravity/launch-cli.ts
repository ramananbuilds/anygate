...
export function readAntigravityCliVersion(
  binaryPath = findAntigravityCliBinary() ?? undefined
): AntigravityCliVersionResult {
  if (!binaryPath) {
...
export function launchAntigravityCli(env: NodeJS.ProcessEnv, extraArgs: string[]): Promise<number> {
