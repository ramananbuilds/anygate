...
export async function searchWeb(
  ...
  switch (cfg.provider) {
    case 'searxng':
      if (!cfg.searxngUrl)
        throw new Error('ANYGATE_SEARXNG_URL is required for the searxng web search provider')
      return searchSearXNG(query, cfg.searxngUrl, searchOpts)
    case 'brave':
      if (!cfg.apiKey)
        throw new Error('ANYGATE_SEARCH_API_KEY is required for the brave web search provider')
      return searchBrave(query, cfg.apiKey, searchOpts)
    case 'tavily':
      if (!cfg.apiKey)
        throw new Error('ANYGATE_SEARCH_API_KEY is required for the tavily web search provider')
