...
import { CLAUDE_CODE_USER_AGENT, injectClaudeIdentity } from '../../auth/claude-identity.js'
...
export function modelPrefersResponsesApi(modelId: string): boolean {
  ...
  if (lower.startsWith('grok-') && (lower.includes('multi-agent') || lower.includes('multiagent')))
    return true
  return false
}
 ...
 */
const OPENAI_CHAT_COMPLETIONS_ONLY = ['davinci-002', 'babbage-002', 'gpt-3.5-turbo-instruct']
...
export async function createLanguageModel(spec: ProviderModelSpec): Promise<LanguageModel> {
  ...
  if (npm === '@ai-sdk/openai') {
    ...
    return shouldUseOpenAiResponsesEndpoint(modelId)
