...
import { upstreamModelId, gatewayProviderLabel, buildDedupedModelRows } from './models.js'
...
export function formatModelCatalogLines(
  ...
  for (const [label, groupModels] of sortedGroups) {
    ...
    const duplicateNote =
      hiddenDuplicates > 0
        ? `, ${hiddenDuplicates} duplicate${hiddenDuplicates !== 1 ? 's' : ''} hidden`
        : ''
    const nameWidth = cappedWidth(
      rows.map(row => row.name),
      'Model',
      28
    )
    const anthropicWidth = cappedWidth(
      rows.map(row => row.anthropicId),
      'Anthropic ID',
      46
    )
    ...
    lines.push(
      `  ${'#'.padStart(indexWidth)}  ${'Model'.padEnd(nameWidth)}  ${'Anthropic ID'.padEnd(anthropicWidth)}  OpenAI ID`
    )
    for (let i = 0; i < rows.length; i++) {
      const row = rows[i]!
      lines.push(
        `  ${String(i + 1).padStart(indexWidth)}  ${row.name.padEnd(nameWidth)}  ${row.anthropicId.padEnd(anthropicWidth)}  ${row.openaiId}`
      )
    }
    lines.push('')
  }
...
export function providerOptionsFromCatalog(
  catalog: import('../../types/index.js').LocalProvider[]
...
async function getServerPasswordForQuickMode(
  ...
  p.log.info(
    'Run `anygate server` and choose Configure & start to save one, or pass a one-run password.'
  )
  return undefined
}
...
function applyServerRunOverrides(
...
async function configureExposedProviders(): Promise<string[] | null | undefined> {
  p.log.info(
    'Add providers to expose. Listed providers are removed when selected — like favorites.'
  )
  ...
  p.log.success(
    `Saved ${picked.length} provider${picked.length !== 1 ? 's' : ''} for future server runs.`
  )
  return picked
}

async function runServerWizard(): Promise<
  { runConfig: ServerRunConfig; promptForPassword: boolean } | undefined
> {
  gateIntro('Server')
...
export async function resolveServerUpstreamApiKey(): Promise<string | null> {
  ...
  apiKey = sanitizeCredential(
    await readFromCredentialStore(reason => {
...
export async function runServerCommand(options: ServerCommandOptions = {}): Promise<number> {
  ...
  if (!apiKey) {
    p.log.error(
      'No providers configured. Run `anygate providers add` or import, or set OPENCODE_API_KEY for Zen/Go.'
    )
    return 1
  }
  ...
  try {
    ...
    spinner.stop(
      `Loaded ${models.length} models (${localCount} from registry providers)${filterNote}${favoritesNote}${freeNote}${maskNote}`
    )
