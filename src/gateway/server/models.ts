...
export function formatAnthropicModelList(entries: ModelDisplayEntry[]) {
  return {
    data: entries.map(entry =>
      formatAnthropicModelEntry(entry.id, entry.name, entry.contextWindow, entry.inputTypes)
    ),
    has_more: false,
...
export function formatGatewayAnthropicModels(
...
export function supportsDirectOpenAIChatCompletions(model: ServerModelInfo): boolean {
  return (
    ...
    (!!model.completionsUrl || model.sourceBackend === 'zen' || model.sourceBackend === 'go')
  )
}
