 ...
 */
const RESTRICTED_OPENAI_PROVIDERS = new Set(['openai', 'openai-oauth'])

/**
 * Google/Vertex provider IDs whose native model IDs (gemini-*) are not
 * directly testable from the UI.
 */
const RESTRICTED_GOOGLE_PROVIDERS = new Set(['google', 'vertex'])
...
export function deriveReasoning(model: UiProviderModel): boolean {
  ...
  return /(opus|sonnet|o1|o3|o4|gpt-5|deepseek-r(1|2)|qwen3?-(plus|max|pro)|claude-(3-7|4))/.test(
    id
  )
}
