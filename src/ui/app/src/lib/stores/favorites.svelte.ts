...
export const favorites = $state<FavoritesState>({
  general: [],
  agy: [],
  loading: false,
  error: null,
...
async function persist(): Promise<void> {
  await api.saveConfig({
    favoriteModels: favorites.general,
    antigravityCliFavoriteModels: favorites.agy,
  })
}
...
export async function removeFavorite(
  ...
  if (agy)
    favorites.agy = favorites.agy.filter(
      f => !(f.providerId === providerId && f.modelId === modelId)
    )
  else
    favorites.general = favorites.general.filter(
      f => !(f.providerId === providerId && f.modelId === modelId)
    )
  await persist()
}
