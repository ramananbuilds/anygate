import {
  ...
  detectApp,
} from '../apps/shared/native-launcher.ts'
...
} from '../registry/templates/add-template.js'
import {
  addCustomEndpointProvider,
  type CustomEndpointKind,
} from '../registry/storage/custom-endpoint.js'
...
import { guiCallbackRedirectUri } from '../auth/claude-code.js'
import { buildAntigravityAuthUrl, completeAntigravityExchange } from '../auth/antigravity-oauth.js'
...
export type UiServerLifecycleEvent =
  { type: 'started'; listenMode: 'local' | 'network'; modelCount: number } | { type: 'stopped' }
...
export function handleUiApiRequest(
  req: IncomingMessage,
  res: ServerResponse,
  opts: UiApiOptions = {}
): void {
...
async function handleGetModels(res: ServerResponse): Promise<void> {
  try {
    ...
    const rawCountById = new Map(
      registry.providers.map(p => [p.id, p.modelsCache?.models.length ?? 0])
    )
...
async function handleRefreshAll(res: ServerResponse): Promise<void> {
  try {
    ...
    sendJson(res, 200, {
      ok: true,
      providers: summary,
      total: summary.reduce((n, p) => n + p.count, 0),
    })
  } catch (err) {
...
async function handleOAuthStart(req: IncomingMessage, res: ServerResponse): Promise<void> {
  try {
    ...
    if (providerId === 'github-copilot') {
      ...
      pollGithubDeviceCodeToken(device)
        .then(async tokens => {
          await saveNativeOAuthCredential(providerId, tokens)
          await refreshOAuthProviderModels(providerId)
          oauthSessions.set(sessionId, { ...session, status: 'done' })
