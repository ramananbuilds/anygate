...
import { parseStoredOAuthCredential } from '../auth/types.js'
...
export type ParsedAuthRef = { kind: 'keyring'; account: string } | { kind: 'env'; varName: string }
...
export async function readGlobalOpencodeCredential(
  diag?: (msg: string) => void
): Promise<string | null> {
 ...
 */
export async function upgradeGlobalOpencodeCredential(
  diag?: (msg: string) => void
): Promise<boolean> {
...
export async function resolveProviderOAuthAccountId(
  ...
  const parsed = parseAuthRef(authRef)
  if (!parsed || parsed.kind !== 'keyring' || !oauthProviderIdFromAccount(parsed.account))
    return undefined
...
export async function readFromCredentialStore(
  diag?: (msg: string) => void
): Promise<string | null> {
  return readGlobalOpencodeCredential(diag)
}

export async function saveToCredentialStore(
  key: string,
  diag?: (msg: string) => void
): Promise<boolean> {
