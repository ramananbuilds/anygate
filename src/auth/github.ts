...
export async function requestGithubDeviceCode(): Promise<GithubDeviceCodeResponse> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `GitHub device code request failed (${response.status})${detail ? `: ${detail}` : ''}`
    )
  }
...
export async function exchangeForCopilotToken(ghuToken: string): Promise<OAuthTokenResponse> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `GitHub Copilot token exchange failed (${response.status})${msg ? `: ${msg}` : ''}`
    )
  }
  const json = (await response.json()) as { token?: string; expires_at?: string }
  if (!json.token) {
    throw new Error(
      'GitHub Copilot token exchange response missing token field — is Copilot subscription active?'
    )
  }
...
export async function pollGithubDeviceCodeToken(
  ...
  while (now() < deadline) {
    ...
    if (error === 'expired_token') {
      throw new Error(
        'GitHub device code expired — please run anygate providers auth github-copilot again'
      )
    }
