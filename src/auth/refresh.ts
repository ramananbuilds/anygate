...
export function oauthCredentialShouldRefresh(
  ...
  if (
    (NATIVE_OAUTH_PROVIDER_IDS as readonly string[]).includes(providerId) &&
    accessTokenIsExpiring(cred.access)
  )
    return true
  return false
}
...
export async function refreshStoredOAuthCredential(
  ...
  if (!cred.refresh) {
    throw new Error(
      `${providerId}: OAuth refresh token missing — run anygate providers auth ${providerId}`
    )
  }
