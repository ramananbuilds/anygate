...
export async function buildAntigravityAuthUrl(redirectUri: string): Promise<AntigravityPkceParams> {
...
async function fetchFirstOk(paths: string[], init: RequestInit): Promise<Response> {
  let lastErr: unknown
...
async function onboardUser(accessToken: string, tierId: string, maxAttempts = 10): Promise<string> {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    if (result.done === true) {
      const p = result.response
        ? (result.response as Record<string, unknown>).cloudaicompanionProject
        : undefined
      if (typeof p === 'string') finalProjectId = p.trim()
      else if (p && typeof p === 'object')
        finalProjectId = String((p as Record<string, unknown>).id ?? '') || finalProjectId
      break
    }
...
async function runBootstrap(tokens: OAuthTokenResponse): Promise<AntigravityOAuthResult> {
