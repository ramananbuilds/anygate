...
import { CODEX_RESPONSES_WEBSOCKETS_BETA } from '../config/constants.js'
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        socket.on('close', (code: number, reason: Buffer) => {
          debug(
            `close code=${code} frames=${frameCount}${reason?.length ? ` reason=${reason.toString('utf8').slice(0, 200)}` : ''}`
          )
          if (closed) return
