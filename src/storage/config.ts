...
export function savePreferences(
  prefs: Partial<
    Pick<
      ...
      | 'antigravityCliFavoritesHintShown'
      | 'appPathOverrides'
      | 'recentLaunchFolders'
    >
  >
  ...
  if (prefs.lastAntigravityProvider !== undefined)
    config.lastAntigravityProvider = prefs.lastAntigravityProvider
  if (prefs.lastAntigravityModel !== undefined)
    config.lastAntigravityModel = prefs.lastAntigravityModel
  if (prefs.recentModelsByProvider !== undefined)
    config.recentModelsByProvider = prefs.recentModelsByProvider
  if (prefs.favoriteModels !== undefined) config.favoriteModels = prefs.favoriteModels
  if (prefs.antigravityCliFavoriteModels !== undefined)
    config.antigravityCliFavoriteModels = prefs.antigravityCliFavoriteModels
  if (prefs.antigravityCliFavoritesHintShown !== undefined)
    config.antigravityCliFavoritesHintShown = prefs.antigravityCliFavoritesHintShown
...
export function recordLaunchFolder(folder: string): string[] {
  ...
  const next = [trimmed, ...prev.filter(path => path !== trimmed)].slice(
    0,
    MAX_RECENT_LAUNCH_FOLDERS
  )
...
export function recordLaunchSelection(
  ...
  const updatedRecent = [modelId, ...prevRecent.filter(id => id !== modelId)].slice(
    0,
    MAX_RECENT_MODELS
  )
  savePreferences({
