...
} from '../../../src/gateway/antigravity/cloud-code-gateway.js'
...
export function formatAgyCapacityWarning(
  validatedSlotCount: number,
  skippedFavoriteCount: number
): string {
...
async function pickAntigravityCliFavoriteLaunchModel(
  ...
  const resolved = favorites
    ...
    .filter(
      (entry): entry is { provider: LocalProvider; model: LocalProviderModel } => entry !== null
    )
...
async function runAntigravityCommand(
  ...
  launch: (
    ...
    routes: ReturnType<typeof buildAntigravityRoutes>,
    gatewayHandle: CloudCodeGatewayHandle
  ) => Promise<number>,
  opts: {
    childArgs?: string[]
...
async function launchWithSelection(
  ...
  const logFn = trace
    ? (msg: string) => {
        try {
          appendFileSync(traceLogPath, `${msg}\n`)
        } catch {}
      }
    : undefined
