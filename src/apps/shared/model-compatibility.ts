...
import blacklistData from '../../registry/data/model-incompatible.json' with { type: 'json' }
...
export type CompatibilityAgent =
  'claude' | 'codex' | 'codex-app' | 'server' | 'gemini' | 'antigravity'
...
function matchesAgent(
  entryAgents: CompatibilityAgent[] | undefined,
  agent: CompatibilityAgent
): boolean {
...
export function readModelsFromCache(backendId: 'zen' | 'go'): Map<string, ModelInfo> | null {
  ...
  for (const entry of Object.values(providerData.models)) {
    ...
    const isFree = entry.cost !== undefined && entry.cost.input === 0 && entry.cost.output === 0
...
export function mergeModels(
  ...
  return uniqueIds
    ...
    .map(id => {
      ...
      if (cached) {
        const modelFormat =
          backendId === 'go' && cached.modelFormat === 'anthropic' ? 'openai' : cached.modelFormat
...
export async function getModels(
  ...
  try {
    ...
    throw new Error('Cannot fetch models. Check your network and https://opencode.ai status.')
  }
}
