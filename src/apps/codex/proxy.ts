...
export async function startCodexProxy(
  ...
  return new Promise((resolve, reject) => {
    const log = debug ? makeTraceLogger(getCodexProxyDebugLogPath()) : () => {}
      ...
      if (req.method === 'POST' && url === '/v1/responses') {
        ...
        try {
          ...
          if (debug) {
            ...
            log(
              `Error: Invalid JSON body on POST ${url}: ${formatUpstreamError(err)} headers=${headers} rawBody=${JSON.stringify(rawBody.slice(0, 2000))}`
            )
          }
          sendJson(res, 400, {
        ...
        if (debug) {
          ...
          const mcpTools = tools.filter(
            (t: unknown) =>
              ...
              typeof t === 'object' &&
              'name' in t &&
              String((t as { name: unknown }).name).startsWith('mcp__')
          )
      ...
      const onData = (chunk: Buffer) => {
        ...
        void (async () => {
          ...
          try {
            ...
            if (debug)
              log(
                `WS Error: Invalid JSON body: rawBody=${JSON.stringify(frame.text.slice(0, 2000))}`
              )
            sendWsEvent(
