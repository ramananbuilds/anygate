...
import { existsSync, readFileSync, mkdirSync, openSync, writeSync, closeSync, chmodSync } from 'node:fs';
import { dirname } from 'node:path';
import { getAppHome } from '../../config/paths.ts';
import { VALIDATION_CONFIG, type ValidationResult, type ValidationCache, type ValidateModelParams, type ValidateModelsOptions } from './config.js';
...
export function isCacheFresh(result: ValidationResult, ttlMs: number = VALIDATION_CONFIG.TTL_MS): boolean {
  try {
...
export function getValidationStatus(providerId: string, modelId: string, ttlMs?: number): ValidationResult | null {
...
function interpretStatus(httpStatus: number): { status: ValidationResult['status']; error?: string } {
 ...
 */
export async function quickValidateModel(
  providerId: string,
  modelId: string,
): Promise<boolean> {
