...
export async function loadRegistryProviders(
  ...
  await Promise.all(
    registry.providers.map(async provider => {
      try {
        ...
        diag?.(
          `${provider.id}: credential unavailable — ${err instanceof Error ? err.message : String(err)}`
        )
      }
  ...
  return materializeRegistry(registry, provider => keys.get(provider.id) ?? null, opts).map(
    provider => ({
      ...provider,
