...
import {
  ...
  upgradeOAuthOpenAiProvider,
  upgradeOAuthXaiProvider,
} from '../upgrade.js'
...
function parseProvider(raw: unknown): RegistryProvider | null {
  ...
  if (
    p.subscriptionFilter === 'free' ||
    p.subscriptionFilter === 'zen' ||
    p.subscriptionFilter === 'go'
  ) {
    ...
    if (typeof cache.fetchedAt === 'string' && Array.isArray(cache.models)) {
      provider.modelsCache = {
        ...
        models: cache.models.filter(
          m => m && typeof m === 'object'
        ) as RegistryProvider['modelsCache'] extends infer C
          ? C extends { models: infer M }
            ? M
            : never
          : never,
      }
