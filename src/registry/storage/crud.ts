...
export async function removeProviderFromRegistry(
  ...
  if (opts?.deleteCredential !== false) {
    ...
    const isGlobal =
      parsed?.kind === 'keyring' && parsed.account === GLOBAL_OPENCODE_KEYRING_ACCOUNT
    const shouldDelete =
      !isGlobal || !credentialStillReferenced(removedProvider.authRef, registry.providers)
...
export function addZenRegistryStub(opts?: { subscriptionFilter?: RegistrySubscriptionFilter }): {
  added: boolean
  reason?: string
} {
  const registry = loadRegistry()
...
export function toggleProviderEnabled(id: string): {
  toggled: boolean
  enabled?: boolean
  error?: string
} {
