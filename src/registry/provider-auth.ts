...
import { runClaudeCodeOAuthFlow, generateCliUserID } from '../auth/claude-code.js'
...
async function runNativeBrowserOAuth(
  providerId: NativeOAuthProviderId
): Promise<OpencodeOAuthCredential> {
  ...
  if (providerId === 'claude-code') {
    ...
    try {
      const { tokens, bootstrap } = await runClaudeCodeOAuthFlow(
        url => {
          spinner.stop('')
...
export async function saveNativeOAuthCredential(
  ...
  if (!saved)
    throw new Error(
      `Could not save OAuth tokens to Keychain${diagMsg ? ` — ${diagMsg}` : ' — grant access and try again'}`
    )
        ...
        if (converted) {
          entry = {
            ...
            api: {
              ...converted.api,
              ...(template?.headers
                ? { headers: { ...template.headers, ...converted.api.headers } }
                : {}),
            },
          }
        }
...
export async function authenticateProvider(
  ...
  if (!supportsNativeOAuth(providerId)) {
    if (findOpencodeBinary()) {
      ...
      if (!saved) {
        p.log.warn(
          `Could not save OAuth tokens to Keychain — ${brokerDiagMsg || 'session may not persist.'}`
        )
      }
  ...
  if (isBrowserRedirectOAuth(providerId)) {
    if (method === 'broker') {
      throw new Error(
        `Via OpenCode is not supported for "${providerId}". Use the built-in OAuth flow.`
      )
    }
    method = 'native'
  }
    ...
    if (hasOpencode) {
      const choice = await p.select({
        ...
        options: [
          {
            value: 'native',
            label: 'Device code (recommended)',
            hint: 'Works on SSH/VPS — open URL on any device',
          },
  ...
  if (!saved) {
    p.log.warn(
      `Could not save OAuth tokens to Keychain — ${nativeDiagMsg || 'session may not persist.'}`
    )
  }
