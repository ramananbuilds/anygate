...
describe('classifyKeyringError', () => {
  it('identifies missing native module', () => {
    expect(classifyKeyringError(new Error("Cannot find module '@napi-rs/keyring'"))).toContain(
      'native keyring module'
    )
    expect(classifyKeyringError(new Error('Module not found: keyring.node'))).toContain(
      'native keyring module'
    )
    expect(classifyKeyringError(new Error('failed to load native binding'))).toContain(
      'native keyring module'
    )
  })

  it('identifies Secret Service / D-Bus daemon not running', () => {
    expect(classifyKeyringError(new Error('Secret Service error: no daemon running'))).toContain(
      'Secret Service daemon'
    )
    expect(classifyKeyringError(new Error('DBus error: connection refused'))).toContain(
      'Secret Service daemon'
    )
    expect(classifyKeyringError(new Error('daemon not available'))).toContain(
      'Secret Service daemon'
    )
  })
...
describe('provider credentials', () => {
  it('parses authRef strings', () => {
    expect(parseAuthRef('keyring:provider:groq')).toEqual({
      kind: 'keyring',
      account: 'provider:groq',
    })
    expect(parseAuthRef('keyring:global:opencode')).toEqual({
      kind: 'keyring',
      account: GLOBAL_OPENCODE_KEYRING_ACCOUNT,
    })
    expect(parseAuthRef('env:OPENCODE_API_KEY')).toEqual({
      kind: 'env',
      varName: 'OPENCODE_API_KEY',
    })
...
describe('buildChildEnv', () => {
  ...
  it('sets ANTHROPIC_AUTH_TOKEN to the same key (for Claude Code v2.x auth check)', () => {
    const env = buildChildEnv(BACKENDS.zen.baseUrl, 'claude-sonnet-4-6', 'my-key')
  ...
  it('sets CLAUDE_CODE_MAX_CONTEXT_TOKENS from model id for proxy sessions', () => {
    expect(
      buildChildEnv(BACKENDS.zen.baseUrl, 'zzzz-unknown-model', 'k')[
  ...
  it('uses explicit contextWindow override when provided', () => {
    expect(
      buildChildEnv(BACKENDS.zen.baseUrl, 'custom-model', 'k', undefined, 512_000)[
        'CLAUDE_CODE_MAX_CONTEXT_TOKENS'
      ]
    ).toBe('512000')
    expect(
      buildChildEnv(BACKENDS.zen.baseUrl, 'custom-model', 'k', undefined, 1_048_576)[
        'CLAUDE_CODE_MAX_CONTEXT_TOKENS'
      ]
    ).toBe('1048576')
  })
