...
import { codexHelpText } from '../../src/apps/codex/cli.js'
import { codexAppHelpText } from '../../src/apps/codex/app.js'
import { claudeAppHelpText } from '../../src/apps/claude/desktop.js'
...
describe('parseArgs', () => {
  ...
  it('consumes --with-claude flag for claude', () => {
    expect(parseArgs(['claude', '--with-claude'])).toMatchObject({
      command: 'claude',
  ...
  it('consumes --all-models flag for claude', () => {
    expect(parseArgs(['claude', '--all-models'])).toMatchObject({
      command: 'claude',
      launchAllModels: true,
      claudeArgs: [],
    })
  })

  it('combines --provider, --model, --all-models', () => {
    expect(parseArgs(['claude', '--provider', 'groq', '--all-models'])).toMatchObject({
      command: 'claude',
      launchProvider: 'groq',
      launchAllModels: true,
      claudeArgs: [],
  ...
  it('consumes gateway launch flags for claude and codex', () => {
    expect(
      parseArgs(['claude', '--provider', 'groq', '--model', 'llama-3.3-70b', '-p', 'hi'])
    ).toMatchObject({
      command: 'claude',
      launchProvider: 'groq',
    ...
    expect(
      parseArgs(['codex', '--provider=zen', '--model=deepseek-v4-flash-free', 'exec', 'fix'])
    ).toMatchObject({
      command: 'codex',
      launchProvider: 'zen',
...
describe('help text', () => {
  ...
  it('claude help includes starter options, providers, and switch menu', () => {
    ...
    expect(help).toContain('--all-models')
    expect(help).toContain('--with-claude')
...
describe('parseArgs — antigravity commands', () => {
  ...
  it('consumes gateway launch flags for antigravity app', () => {
    expect(
      parseArgs(['antigravity', '--provider=zen', '--model=deepseek-v4-flash-free', '--wait'])
    ).toMatchObject({
      command: 'antigravity',
      launchProvider: 'zen',
      launchModel: 'deepseek-v4-flash-free',
  ...
  it('consumes gateway launch flags for agy', () => {
    expect(
      parseArgs(['agy', '--provider=zen', '--model=deepseek-v4-flash-free', '-p', 'fix'])
    ).toMatchObject({
      command: 'agy',
      launchProvider: 'zen',
      launchModel: 'deepseek-v4-flash-free',
  ...
  it('passes antigravity-ide passthrough args unchanged', () => {
    expect(parseArgs(['antigravity-ide', '--user-data-dir', '/tmp/test']).claudeArgs).toEqual([
      '--user-data-dir',
      '/tmp/test',
    ])
  })
