...
vi.mock('../../src/gateway/proxy/anthropic-proxy.js', async importOriginal => {
  const actual = await importOriginal<typeof import('../../src/gateway/proxy/anthropic-proxy.js')>()
  return {
    ...actual,
    startProxyCatalog: vi
      .fn()
      .mockResolvedValue({ port: 49999, token: 'proxy-token-xyz', close: vi.fn() }),
  }
})
...
import { startProxyCatalog } from '../../src/gateway/proxy/anthropic-proxy.js'
...
describe('needsCloudCodeBackend', () => {
  it('selects cloud-code models and OAuth Anthropic models only', () => {
    ...
    expect(
      needsCloudCodeBackend({ ...anthropicOAuthModel, modelFormat: 'anthropic' }, 'oauth')
    ).toBe(true)
    expect(needsCloudCodeBackend({ ...anthropicOAuthModel, modelFormat: 'anthropic' }, 'api')).toBe(
      false
    )
...
describe('buildSingleModelCloudCodeRoute', () => {
  ...
  it('starts a one-route backend for a cloud-code model', async () => {
    const result = await buildSingleModelCloudCodeRoute(
      model,
      'tok',
      'antigravity',
      { projectId: 'p' },
    ...
    expect(startProxyCatalog).toHaveBeenCalledWith(
      [result.proxyRoute],
      result.proxyRoute.aliasId,
      false,
      { app: undefined }
    )
    expect(result.proxyRoute).toMatchObject({
      providerId: 'antigravity',
      modelFormat: 'cloud-code',
    })
    expect(result.backend).toMatchObject({ port: 49999, token: 'proxy-token-xyz' })
