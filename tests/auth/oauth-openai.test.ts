...
describe('auth/openai', () => {
  ...
  describe('refreshOpenAiAccessToken', () => {
    ...
    it('throws on non-ok response', async () => {
      ...
      await expect(refreshOpenAiAccessToken('refresh_123')).rejects.toThrow(
  ...
  describe('runOpenAiDeviceCodeFlow', () => {
    ...
    it('throws if device initiation fails', async () => {
      ...
      await expect(runOpenAiDeviceCodeFlow(vi.fn())).rejects.toThrow(
        'Failed to initiate OpenAI device authorization'
      )
    })
    ...
    it('throws if polling hits an unexpected error (e.g. 500)', async () => {
      ...
      // 2. Polling fails with 500. `text` is supplied because the poll loop
      //    reads the body to include it in the thrown error — a real Response
      //    always has it.
      vi.mocked(global.fetch).mockResolvedValueOnce({
        ok: false,
        status: 500,
      ...
      await expect(runOpenAiDeviceCodeFlow(vi.fn())).rejects.toThrow(
        'OpenAI device authorization failed (500)'
      )
    })

    it('includes the response body in a terminal polling error', async () => {
      vi.mocked(global.fetch).mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          device_auth_id: 'auth_id',
      ...
      // Without the body, a rejected client id is indistinguishable from any
      // other 400 and the user has nothing to act on.
      await expect(runOpenAiDeviceCodeFlow(vi.fn())).rejects.toThrow(/invalid_client/)
    })
    ...
    it('throws if device authorization times out', async () => {
      ...
      await expect(runOpenAiDeviceCodeFlow(vi.fn(), { sleep, now })).rejects.toThrow(
