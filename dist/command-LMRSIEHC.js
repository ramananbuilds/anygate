...
import { BACKENDS, GATEWAY_PORT, MAX_MODEL_CATALOG } from './chunk-KEFRKMGW.js'
import {
  getTemplateById,
...
function fallbackPathsForApp(id, platform = process.platform) {
  ...
  switch (id) {
    case 'claude':
      return windows
        ? [join(appData, 'npm', 'claude.cmd'), join(appData, 'npm', 'claude')]
        : [
    ...
    case 'codex':
      return windows
        ? [join(appData, 'npm', 'codex.cmd'), join(appData, 'npm', 'codex')]
        : [
    ...
    case 'gemini':
      return windows
        ? [join(appData, 'npm', 'gemini.cmd'), join(appData, 'npm', 'gemini')]
        : [
...
async function doStartGatewayServer(req) {
  ...
  if (req.listenMode === 'network') {
    if (req.passwordMode === 'saved') {
      ...
      if (!saved)
        return { ok: false, error: 'No saved password found \u2014 enter a new password.' }
...
async function handleTestModel(req, res) {
  ...
  try {
    ...
    const format =
      model.modelFormat === 'anthropic' || model.modelFormat === 'openai'
        ? model.modelFormat
        : 'unsupported'
    ...
    if (model.modelFormat === 'anthropic') {
      ...
      sendJson(res, 200, {
        ...
        errorHint:
          "This provider routes through anygate's SDK adapter. Use the CLI or Server gateway to exercise it.",
      })
      return
    }
    ...
    try {
      ...
      clearTimeout(timeout)
      const tokensPerSec =
        tokens > 0 && ttftMs !== null
          ? Math.round((tokens / (totalMs - ttftMs + 1)) * 1e3 * 10) / 10
          : null
...
function failNoEndpoint(providerId, modelId, format) {
  return {
    ...
    errorHint:
      'The provider is missing a base URL / completions URL. Re-add or refresh it in Providers & Keys.',
  }
}
...
var DEVICE_CODE_PROVIDER_IDS = /* @__PURE__ */ new Set([
  'xai-oauth',
  'openai-oauth',
  'github-copilot',
])
var PKCE_PROVIDER_IDS = /* @__PURE__ */ new Set(['claude-code', 'antigravity'])
var NATIVE_OAUTH_PROVIDER_IDS = /* @__PURE__ */ new Set([
...
async function handleOAuthStart(req, res) {
  try {
    ...
    if (providerId === 'xai-oauth') {
      ...
      pollXaiDeviceCodeToken(device2)
        .then(async tokens => {
    ...
    if (providerId === 'github-copilot') {
      ...
      oauthSessions.set(sessionId, session2)
      pollGithubDeviceCodeToken(device2)
        .then(async tokens => {
          await saveNativeOAuthCredential(providerId, tokens)
          await refreshOAuthProviderModels(providerId)
          oauthSessions.set(sessionId, { ...session2, status: 'done' })
    ...
    if (PKCE_PROVIDER_IDS.has(providerId)) {
      if (providerId === 'claude-code') {
        sendJson(res, 400, {
          error:
            'Claude Code OAuth must be completed in the terminal: anygate providers auth claude-code',
        })
        return
      }
      ...
      const codePromise = new Promise((resolve, reject) => {
        ...
        setTimeout(
          () => reject(new Error('OAuth timeout \u2014 sign-in not completed')),
          10 * 60 * 1e3
        )
      })
      oauthSessions.set(sessionId, session2)
    ...
    pollOpenAiDeviceCodeToken(device)
      .then(async ({ tokens, accountId }) => {
  ...
  try {
    ...
    if (isMac2) {
      ...
      try {
        ...
        if (
          err.code === 1 ||
          String(err.stderr).includes('-128') ||
          String(err.stdout).includes('-128')
        ) {
...
async function resolveUiShutdownDecision(
  signal,
  promptClose = () =>
    p.confirm({
...
//# sourceMappingURL=command-LMRSIEHC.js.map
